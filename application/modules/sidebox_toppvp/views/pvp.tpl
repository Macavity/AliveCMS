	...
	{foreach from=$realms key=key item=realm}
		<div class="toppvp_realm" id="toppvp_realm_{$key - 1}" style="display:none;">
			<div class="toppvp_select">
				{if $key != $max_realm}<a href="javascript:void(0)" onClick="TopPvP.show(TopPvP.current + 1)" class="toppvp_next" data-tip="Next realm">&rarr;</a>{/if}
				{if $key != $min_realm && $max_realm != 1}<a href="javascript:void(0)" onClick="TopPvP.show(TopPvP.current - 1)" class="toppvp_previous"  data-tip="Previous realm">&larr;</a>{/if}
				{$realm->getName()}
			</div>
