<section class="box big">
	...
	<form>
		...
		<select id="item_type" name="item_type" onChange="Items.changeType(this)">
			<option value="item" {if !$item.query}selected{/if}>Item</option>
			<option value="query" {if $item.query}selected{/if}>Query</option>
		</select>
	</form>

	<script type="text/javascript">
		var formType = {if $item.query}"query"{else}"item"{/if};
	</script>
	...
	<form onSubmit="Items.save(this, {$item.id}); return false" id="query_form" {if !$item.query}style="display:none;"{/if}>
		...
		<select id="query_database" name="query_database">
			...
			<option value="realm" {if $item.query_database == "realm"}selected{/if}>Realm</option>
			<option value="realmd" {if $item.query_database == "realmd"}selected{/if}>Realmd (Accounts/Logon)</option>
		</select>
