...
define(['modules/blackout'], function (Blackout) {
    var Login = {
        ...
        userplateUrl: "/login/userplate/",

        /**
         * Checks for login and modifies the page accordingly
         */
        initialize: function(){
        ...
        open: function(url, legacyHref, legacyReferrer) {
            ...
            if ($("#"+Login.CONTAINER_ID).length > 0) {
                return false;
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    debug.debug("login dialog sucess");
                    ...
                    $("#login-embedded").parent().find(".ui-dialog-titlebar").hide();
                    $("#login-embedded").find("#embedded-close").attr("onclick", "return Login._close(true);");
                },
                error: function(){
                    debug.debug("login dialog error");
                    //document.location = legacyHref + encodeURIComponent(legacyReferrer + delim + "ST-frag=" + encodeURIComponent(Core.getHash()));
                }
            });
